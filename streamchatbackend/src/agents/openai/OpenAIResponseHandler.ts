...
export class OpenAIResponseHandler{
     ...
     if(textDelta?.type === "text" && textDelta.text){
      ...
      this.chatClient.partialUpdateMessage(id,{
        set:{
          ...
          event.data.content[0].text.value : this.message_text,
        }
      });
  ...
  private handleError = async(error:Error) =>{
    ...
    await this.chatClient.partialUpdateMessage(this.message.id,{
      set:{
        text:error.message ?? "Error generating message",
  ...
  private performWebSearch = async(query:string): Promise<string> =>{
    ...
    if(!TAVILY_API_KEY){
      return JSON.stringify({
        error:"Web search is not availabale, API key not configured",
      });
    }
    ...
    try {
     ...
     if(!response.ok){
      ...
      console.log(`Tavily search failed for queary "${query}: `,errorText);
     ...
     console.log(`Tavily search successful for queary: ${query}`);

     return JSON.stringify(data)
    } catch (error) {
      console.error(`An exception occured during the web search for ${query}`);
      return JSON.stringify({
